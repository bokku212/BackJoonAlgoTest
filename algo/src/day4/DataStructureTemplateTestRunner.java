...
public class DataStructureTemplateTestRunner {
    ...
    //260131 LinkedList 테스트코드 강화 버전 코드 추가됨
    ...
    private static void testMyLinkedList_Stronger() {
        ...
        {
            MyLinkedList list = new MyLinkedList();
            for (int i = 0; i < 20; i++) list.addLast(i); // [0..19]
            ...
            for (int i = 19; i >= 0; i--) {
                ...
                if (i > 0) assertEquals(0, list.get(0), "head should still be 0");
            }
        ...
        //    - 이게 가장 강력함: 누락된 꼬리 처리/size/연결 깨짐을 거의 다 잡아냄
        {
            ...
            for (int step = 0; step < 3000; step++) {
                ...
                if (op == 0) { // addFirst
                    ...
                    if (ref.isEmpty()) {
                        assertThrows(NoSuchElementException.class, my::removeFirst, "removeFirst on empty (step " + step + ")");
                    } else {
                    ...
                    if (ref.isEmpty()) {
                        assertThrows(NoSuchElementException.class, my::removeLast, "removeLast on empty (step " + step + ")");
                    } else {
                        int a = my.removeLast();
                    ...
                    if (ref.isEmpty()) {
                        assertThrows(IndexOutOfBoundsException.class, () -> my.get(0), "get(0) on empty (step " + step + ")");
                    } else {
                        int idx = rnd.nextInt(ref.size());
                        assertEquals(ref.get(idx), my.get(idx), "get(" + idx + ") (step " + step + ")");
                    }
                } else { // size
                ...
                if (step % 200 == 0) {
                    for (int i = 0; i < ref.size(); i++) {
                        assertEquals(ref.get(i), my.get(i), "full scan get(" + i + ") at step " + step);
                    }
                }
            }
        }
    ...
    /**
     * ✅ 강화 포인트:
     * - private 필드(head/tail/size)까지 reflection으로 점검해서
     *   "테스트는 통과하지만 내부 구조가 깨진 구현"을 잡아냄
     */
    private static void assertInvariants(MyLinkedList list, String label) {
        try {
            ...
            while (true) {
                ...
                last = cur;
                cnt++;
                if (cnt > size) break; // cycle/size 불일치 방지
